use crate::util::{api, exitcode, node_rpc, OckamConfig, RpcBuilder};
...
async fn run_impl(
    ...
    let port = match cfg.inner().nodes.get(&node_name) {
        None => {
            eprintln!("No such node available.  Run `ockam node list` to list available nodes");
            return Err(crate::error::Error::new(
                exitcode::IOERR,
                anyhow!("No nodes available for node show command"),
            ));
        }
        Some(x) => x.port(),
    };
    ...
    if !is_node_up(&mut child_ctx, &route, true).await? {
